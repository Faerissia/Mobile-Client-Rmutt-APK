...
class UniversityScoreDetail extends StatelessWidget {
  final int uniID;
  const UniversityScoreDetail({Key? key, required this.uniID})
      : super(key: key);

  Future<Map<String, dynamic>> _getallrank() async {
    Uri requestUri = Uri.parse('$ipaddress/allrank/$uniID');
...
Widget _builddetail(BuildContext context, Map<String, dynamic> data) {
  ...
  return Scaffold(
    ...
    appBar: AppBar(
      ...
      title: Text("รายละเอียดเหรียญรางวัล"),
    ),
                      ...
                      child: Row(
                        ...
                        children: [
                          Container(
                            ...
                            child: Text(
                              "${data['rows'][index]['tnmName']}",
                              style: TextStyle(fontSize: 18),
                            ),
                          ),
                          Text(
                            "${data['rows'][index]?['place'] ?? '-'}",
                            style: TextStyle(fontSize: 18),
                          ),
                        ],
                      ),
