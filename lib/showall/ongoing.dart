...
class showongoing extends StatefulWidget {
  const showongoing({super.key});

  @override
  State<showongoing> createState() => _sportState();
}
...
class _sportState extends State<showongoing> {
  List ongoing = [];
  Future<String> getongoing() async {
    Uri requestUri = Uri.parse('${ipaddress}/ongoing');
      ...
      body: Padding(
        ...
        child: Container(
          ...
          child: GridView.builder(
              ...
              itemCount: ongoing.length, //แก้ไขใส่ค่าตาม length ของข้อมูล
              itemBuilder: (BuildContext context, index) {
                return GestureDetector(
                  onTap: () {
                    int tnmID = ongoing[index]['tnmID'];
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                    ...
                    child: Column(
                      children: [
                        Padding(
                          ...
                          child: Text(
                              '${ongoing[index]['tnmName']}', //ตัวอย่างการดึง index มาแสดง แก้ไข SQL NAME Tournamment
                        ...
                        Container(
                            ...
                            child: Image.network(
                              '$ipaddress/assets/images/${ongoing[index]['tnmPicture']}',
                            ...
                            Text(
                                'วันที่เปิดรับสมัคร',
                                style: TextStyle(fontSize: 15)),
                       Text('${ongoing[index]['tnmStartdate']} - ${ongoing[index]['tnmEnddate']}',
                            style: TextStyle(fontSize: 14)),
                        Text('${ongoing[index]['sportPlaynum'] > 1 ? 'ทีมที่เข้าร่วม' : 'ผู้เข้าร่วม'} ${ongoing[index]['nop'] ~/ ongoing[index]['sportPlaynum']} ${ongoing[index]['sportPlaynum'] > 1 ? 'ทีม' : 'คน'}',
                            style: TextStyle(fontSize: 15)),
                      ],
                    ),
                  ),
